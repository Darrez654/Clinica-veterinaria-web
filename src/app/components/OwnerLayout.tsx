/**
 * OwnerLayout - Layout para Dueños de Mascotas
 * Sidebar con acceso a los módulos del dueño
...
import {
  Home, PawPrint, Calendar, FileText, User,
  ChevronLeft, Menu, LogOut, Bell, Heart, Shield
} from 'lucide-react';
...
export function OwnerLayout() {
  ...
  const navItems = [
    { to: '/dueno', label: 'Mis Mascotas', icon: Home, end: true },
    { to: '/dueno/mascotas', label: 'Registrar Mascota', icon: PawPrint },
    { to: '/dueno/citas', label: 'Mis Citas', icon: Calendar },
    { to: '/dueno/expedientes', label: 'Expedientes', icon: FileText },
  ];
          ...
          <div className={`flex items-center ${!sidebarOpen ? 'lg:justify-center' : 'gap-3'}`}>
            ...
            {sidebarOpen && (
              <div className="overflow-hidden">
                ...
                <p className="text-xs text-amber-600 dark:text-amber-400 font-medium">Dueño de Mascota</p>
              </div>
            )}
          </div>
        </div>
        ...
        <header className="bg-card/80 backdrop-blur-sm border-b border-border px-4 lg:px-6 py-3 flex items-center justify-between relative z-50">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-amber-600 dark:text-amber-400 font-medium">Portal del Dueño</span>
            </div>
          </div>
