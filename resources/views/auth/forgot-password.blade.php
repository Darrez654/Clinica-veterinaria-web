...
<head>
    ...
    <title>VetClinic - Recuperar Contraseña</title>
    <style>
        *  { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .success-feedback {
            background: #def2e6; color: #1a7a4a; padding: 12px 14px; border-radius: 10px;
            font-size: 13px; margin-bottom: 20px; border: 1px solid #b8e6c8;
        }
...
<body>
    <div class="login-card">
        ...
        @if (session('status'))
            <div class="success-feedback">{{ session('status') }}</div>
        @endif

        <form method="POST" action="/olvide-contrasena">
            @csrf
